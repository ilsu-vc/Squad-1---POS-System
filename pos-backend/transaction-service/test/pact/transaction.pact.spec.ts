...
describe('Transaction Service Pact Verification', () => {
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .useValue({
        getClient: () => ({
          from: () => ({
            select: () => ({
              eq: () => ({ single: async () => ({ data: null, error: { message: 'not found', code: 'PGRST116' } }) }),
          ...
          rpc: async () => ({ data: [{ o_receipt_number: 'REC-MOCK' }], error: null }),
        }),
      })
