...
describe('TransactionService (Integration) - SCRUM-317, 318, 319', () => {
  ...
  beforeAll(async () => {
    ...
    .useValue({
      getClient: () => ({
        from: () => ({
          insert: () => ({ select: () => ({ single: async () => ({ data: { id: 'test-txn-123' }, error: null }) }) }),
          select: () => ({ eq: () => ({ single: async () => ({ data: { id: 'test-txn-123', status: 'paid' }, error: null }) }) }),
        }),
    ...
    it('should enforce hold limit constraints (e.g. max 10 active holds)', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await request(app.getHttpServer())
          ...
          .send({ label: \`Hold \${i}\`, total: 100, items: [] })
          .expect(201);
      }
      ...
      const res = await request(app.getHttpServer())
          .post('/transactions/hold')
          .send({ label: 'Hold 11', total: 100, items: [] });
